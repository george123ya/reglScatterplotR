...
test_that(".parseLimit handles NULL, numeric, percentile, min, max", {
    v <- c(1, 2, 3, 4, 5, 100)
    expect_equal(reglScatterplotR:::.parseLimit(NULL, v, min), 1)
    expect_equal(reglScatterplotR:::.parseLimit(NULL, v, max), 100)
    expect_equal(reglScatterplotR:::.parseLimit(42, v, min), 42)
    expect_equal(reglScatterplotR:::.parseLimit("min", v, min), 1)
    expect_equal(reglScatterplotR:::.parseLimit("max", v, max), 100)
    expect_equal(
        reglScatterplotR:::.parseLimit("p99", v, max),
        unname(stats::quantile(v, 0.99, na.rm = TRUE))
    )
})

test_that(".parseLimit rejects unrecognised strings", {
    expect_error(reglScatterplotR:::.parseLimit("garbage", 1:3, min))
})

test_that(".normaliseRange maps endpoints to [-1, 1] and clips outside", {
    v <- c(0, 5, 10, -1, 11)
    out <- reglScatterplotR:::.normaliseRange(v, 0, 10)
...
test_that(".normaliseRange handles a degenerate range", {
    expect_equal(
        reglScatterplotR:::.normaliseRange(c(3, 3, 3), 3, 3),
        c(0, 0, 0)
    )
})
...
test_that(".resolveColumn returns vector pass-through and column lookup", {
    ...
    expect_equal(reglScatterplotR:::.resolveColumn("a", df), 1:3)
    expect_equal(reglScatterplotR:::.resolveColumn(1:3, NULL), 1:3)
    expect_null(reglScatterplotR:::.resolveColumn(NULL, df))
})

test_that(".validateCoord errors for non-numeric / empty / NULL", {
    expect_error(reglScatterplotR:::.validateCoord(NULL, "x"))
    expect_error(reglScatterplotR:::.validateCoord(numeric(0), "x"))
    expect_error(reglScatterplotR:::.validateCoord(letters, "x"))
    expect_true(reglScatterplotR:::.validateCoord(1:5, "x"))
})
